...
public class MultiDialogActivity extends BaseActivity
    ...
    public void showListCheckPrompt() {
        new MaterialDialog.Builder(this)
                ...
                .itemsCallback(new MaterialDialog.ListCallback() {
                    @Override
                    public void onSelection(MaterialDialog dialog, View itemView, int position, CharSequence text) {
                        showToast(position + ": " + text + ", prompt: " + dialog.isPromptCheckBoxChecked());
                    }
                })
    ...
    public void showMultiChoiceLimited() {
        new MaterialDialog.Builder(this)
                ...
                .itemsCallbackMultiChoice(new Integer[]{1}, new MaterialDialog.ListCallbackMultiChoice() {
                    @Override
                    public boolean onSelection(MaterialDialog dialog, Integer[] which, CharSequence[] text) {
                        boolean allowSelectionChange = which.length <= 2; // limit selection to 2, the new (un)selection is included in the which
                        // array
    ...
    public void showMultiChoiceLimitedMin() {
        new MaterialDialog.Builder(this)
                ...
                .itemsCallbackMultiChoice(new Integer[]{1}, new MaterialDialog.ListCallbackMultiChoice() {
                    @Override
                    public boolean onSelection(MaterialDialog dialog, Integer[] which, CharSequence[] text) {
                        boolean allowSelectionChange = which.length >= 1; // selection count must stay above 1, the new (un)selection is included
                        // in the which array
                        if (!allowSelectionChange) {
    ...
    public void showFileChooser() {
        ...
        if (ActivityCompat.checkSelfPermission(
                MultiDialogActivity.this, Manifest.permission.READ_EXTERNAL_STORAGE)
                != PackageManager.PERMISSION_GRANTED) {
            ActivityCompat.requestPermissions(
                    MultiDialogActivity.this,
    ...
    public void showFolderChooser() {
        ...
        if (ActivityCompat.checkSelfPermission(
                MultiDialogActivity.this, Manifest.permission.WRITE_EXTERNAL_STORAGE)
                != PackageManager.PERMISSION_GRANTED) {
            ActivityCompat.requestPermissions(
                    MultiDialogActivity.this,
        ...
        new FolderChooserDialog.Builder(MultiDialogActivity.this)
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        getMenuInflater().inflate(R.menu.multi_dialog_menu, menu);
